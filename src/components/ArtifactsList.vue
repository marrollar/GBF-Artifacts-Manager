...
function filterLogic(
  artifact: [string, Artifacts[number]],
  filterOpts: ActiveFilters,
) {
...
function filterAndSort(artifacts: Artifacts, filterOpts: ActiveFilters) {
  var filteredArtifacts = Object.entries(artifacts).filter((arti) =>
    filterLogic(arti, filterOpts),
  );
  ...
  const sortedArtifacts = filteredArtifacts.sort((a, b) => {
    ...
    if (a_elemOrd !== b_elemOrd)
      return (a_elemOrd ?? Infinity) - (b_elemOrd ?? Infinity);

    const a_wepOrd = weaponSortOrder.get(a[1].weapon);
    const b_wepOrd = weaponSortOrder.get(b[1].weapon);

    if (a_wepOrd !== b_wepOrd)
      return (a_wepOrd ?? Infinity) - (b_wepOrd ?? Infinity);
...
<template>
  <div class="flex flex-col gap-2 p-2">
    <div
      ...
      :key="artifact[0]"
      class="flex gap-1 h-[40px] items-center"
    >
      <input
        type="checkbox"
      ...
      <div className="flex h-full w-full">
        <ArtifactSkillColumn
          :name="artifact[1].s1.name"
          :value="artifact[1].s1.value"
        />
        <ArtifactSkillColumn
          :name="artifact[1].s2.name"
          :value="artifact[1].s2.value"
        />
        <ArtifactSkillColumn
          :name="artifact[1].s3.name"
          :value="artifact[1].s3.value"
        />
        <ArtifactSkillColumn
          :name="artifact[1].s4.name"
          :value="artifact[1].s4.value"
        />
      </div>
    </div>
  </div>
