...
import { type Weapon, type Element, elements, weapons } from './types';
...
function filterClearerFactory<K extends keyof ActiveFilters | "all">(key: K) {
  switch (key) {
    case "all":
      return () => {
        filters.search = "";
    ...
    case "sk1Search":
      return () => filters.sk1Search = new Set<string>;
    case "sk2Search":
      return () => filters.sk2Search = new Set<string>;
    case "sk3Search":
      return () => filters.sk3Search = new Set<string>;
    case "element":
      return () => filters.element = new Set<Element>;
    case "weapon":
      return () => filters.weapon = new Set<Weapon>;
        ...
        <div class="flex flex-col items-center gap-2 pr-6">
          <div class="flex flex-col items-center gap-2" v-if="!sidePanelRef?.isCollapsed">
            ...
            <div class="flex items-center justify-center">
              <button class="btn bg-base-100 hover:bg-red-400 h-7" @click="filterClearerFactory('all')()">
                Clear Filters </button>
            </div>
            ...
            <div class="flex gap-2">
              <div class="flex items-center gap-2 h-[50px] overflow-auto">
                <button v-for="element in elements" :key="element"
                  class="flex-none p-0.5 hover:bg-neutral-500 hover:cursor-pointer rounded-md"
                  :class="{ 'bg-accent': filters['element'].has(element) }">
                  <img class="w-[30px] h-[30px]" :src="getImage(`Icon_Element_${element}.png`)"
                    @click="filterUpdaterFactory('element')(element)" />
                </button>

                <div class="flex items-center">
                  <ClearFilterButton filter-name="element" @clear-filter="filterClearerFactory('element')()" />
                </div>
              </div>
            </div>
            ...
            <div class="flex gap-2">
              <div class="flex overflow-auto">
                <div class="flex-none grid grid-flow-row grid-rows-2 grid-cols-5 overflow-auto pb-2">
                  <button v-for="weapon in weapons" :key="weapon"
                    class="p-0.5 hover:bg-neutral-500 hover:cursor-pointer rounded-md"
                    :class="{ 'bg-accent': filters['weapon'].has(weapon) }">
                    <img class="h-[20px]" :src="getImage(`Label_Weapon_${weapon}.png`)"
                      @click="filterUpdaterFactory('weapon')(weapon)" />
                  </button>
                </div>
              </div>
              <div class="flex items-center pb-2">
                <ClearFilterButton filter-name="weapon" @clear-filter="filterClearerFactory('weapon')()" />
              </div>
            </div>
